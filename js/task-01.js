...
const childCategories = categories.children.length;
console.log(`Number of categories: ${childCategories}`);
const categoryItem = document.querySelectorAll(".item");

categoryItem.forEach(element => {
  console.log(`Category: ${element.firstElementChild.textContent}`);
  let listCounter = element.lastElementChild.querySelectorAll('li');
  console.log(`Elements: ${listCounter.length}`);
});
